...
// (formatPace/formatClock re-eksporteres av running.ts for bruk i /lop)
...
function WeightCard() {
  ...
  const pts = (vals: number[]) =>
    vals
      .map(
        (v, i) => `${(i / Math.max(1, vals.length - 1)) * 100},${100 - ((v - min) / span) * 100}`,
      )
      .join(" ");
...
function CheckinCard() {
    ...
    return (
      <section className="rounded-3xl border border-border bg-card p-5 shadow-soft">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Søvnscore {existing.sleepScore}/100 · {existing.recoveryHours}t til restituert · ømhet{" "}
          {existing.soreness}/5
        </p>
      </section>
    );
  ...
  function save() {
    const sleepN = parseInt(sleep, 10);
    const hoursN = parseInt(recoveryHours, 10);
    if (!Number.isFinite(sleepN) || sleepN < 0 || sleepN > 100) return;
    if (!Number.isFinite(hoursN) || hoursN < 0 || !soreness) return;
    setState((s) => ({
      ...s,
      checkins: [
        ...s.checkins.filter((c) => c.date !== today),
        { date: today, sleepScore: sleepN, recoveryHours: hoursN, soreness },
      ],
    }));
  }
    ...
    <section className="space-y-3 rounded-3xl border border-border bg-card p-5 shadow-soft">
      ...
      <div>
        <p className="mb-1 text-xs font-bold text-muted-foreground uppercase">
          Timer til fullstendig restituert
        </p>
        <input
          inputMode="numeric"
          value={recoveryHours}
          onChange={(e) => setRecoveryHours(e.target.value)}
          placeholder="Tall fra Garmin Recovery Time"
          className="tap-target w-full rounded-2xl border border-input bg-background px-4 text-lg font-semibold outline-none focus:border-primary"
        />
      </div>
      <div>
...
function RunCard() {
  ...
  const [saved, setSaved] = useState(false);
  const doneToday = state.runLogs.some((r) => r.date === today);

  if (!type) return null;

  const prescription = nextRunPrescription(type, state.runPlan, state.runLogs);

  function save() {
    const km = parseFloat(distance.replace(",", "."));
    const durationSec = (parseInt(minutes, 10) || 0) * 60 + (parseInt(seconds, 10) || 0);
    if (!Number.isFinite(km) || km <= 0 || durationSec <= 0 || !type) return;
    const avgPaceSecPerKm = paceSecPerKm(km, durationSec);
    setState((s) => ({
      ...s,
      runLogs: [
        ...s.runLogs,
  ...
  return (
    <section className="space-y-3 rounded-3xl border border-border bg-card p-5 shadow-soft">
      <h2 className="flex items-center gap-2 text-lg font-bold">
        <Footprints className="size-5 text-primary" /> Dagens løpsøkt – {RUN_TYPE_NAMES[type]}
      </h2>
      <p className="text-sm text-muted-foreground">{prescription.description}</p>

      {doneToday ? (
        <p className="text-sm font-semibold text-success">Løpetur logget i dag ✓</p>
      ) : (
        <>
          <div className="flex gap-2">
            ...
            <div className="w-20">
              <p className="mb-1 text-xs font-bold text-muted-foreground uppercase">Min</p>
              <input
                inputMode="numeric"
                value={minutes}
                onChange={(e) => setMinutes(e.target.value)}
                className="tap-target w-full rounded-2xl border border-input bg-background px-2 text-lg font-semibold outline-none focus:border-primary"
              />
            </div>
            <div className="w-20">
              <p className="mb-1 text-xs font-bold text-muted-foreground uppercase">Sek</p>
              <input
                inputMode="numeric"
                value={seconds}
          ...
          <button
            ...
            className="tap-target gradient-hero shadow-pop press w-full rounded-2xl text-base font-bold text-primary-foreground active:scale-95"
          >
            {saved ? <Check className="mx-auto size-5 animate-pop-in" /> : "Logg løpetur"}
          </button>
        </>
      )}
    </section>
  ...
  return (
    <div className="space-y-5">
      ...
      <Link
        ...
        className="flex items-center justify-center gap-2 rounded-2xl border border-border bg-card p-4 text-base font-bold text-primary"
      >
        <Footprints className="size-5" /> Se løpeplan og utvikling
      </Link>
